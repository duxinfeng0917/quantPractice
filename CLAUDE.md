...
STOCK=02513 bash start.sh all            # 质谱（02513）启动
...
- **`shared_config.py`**: STOCKS dict (per-stock symbol, db_path, poll_interval) + shared constants. Add new stocks here.

- **`short_data.db`**: shared SQLite; tables: `hkex_daily`, `capital_flow`, `orderbook_snapshots`, `price_history`, `signals`, `monitor_state`.
