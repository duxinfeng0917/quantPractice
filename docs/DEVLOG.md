...
## 迭代四十八：冰山价格闸 ±0.10%→±0.25%，修高价票"价守位"判定结构性偏紧（2026-06-12）

### 动机（实盘观察）

06-12 00100 在 402 钉扎被接 → 弹至 412.6（+2.4%）。`analyze_iceberg_absorption` 的买侧吸筹判定
要求窗口价 `price_pct ≥ ICEBERG_PRICE_FLOOR(-0.10%)`，但 **-0.10% 对 400+ 港元的高价票结构性偏紧**：

- 0.10% × 407 ≈ 0.4 HKD ≈ **2 个 tick**（该价位 tick=0.2）。4 轮窗口（约 1 分钟）价格晃几跳就破闸。
- 实盘 10:55 段主卖 4×、价 406→412 横盘前，窗口 407.8→406.8 ≈ **-0.25%** 被判成"在跌→真卖压"，
  冰山全程未报，漏掉了一次被接住后转轧空的吸筹。

这是**标定问题**（高价票的"价守位"容忍度被换算成了过少的 tick），非灵敏度问题。

### 改动（`short_squeeze_monitor.py`）

`ICEBERG_PRICE_FLOOR` −0.10→**−0.25**，`ICEBERG_PRICE_CAP` +0.10→**+0.25**（@407 ≈ 5 ticks/≈1.0 HKD，
仍属"基本守位"）。**只改"价守位"判定口径，不动 score 权重**——买侧吸筹分已按迭代四十五降权至 8/4
（回测看多预测力弱、0/3 跨标的一致性），放大弱信号无益；故仅修判定门，分值维持。

### 未动 & 后续

- **未动 `ICEBERG_MIN_NOTIONAL(1.5M)`**：当日 402 真钉扎段（10:35–10:38）窗口主动卖仅 1,100–1,760 股
  ≈ 44–71 万 HKD，是被**成交额门槛**挡的（价格闸过），价格闸放宽对其无效。
- 后续可考虑**钉扎耦合**：当 |price_pct| 极小（强控盘证据）时按比例下调 notional 门槛，以捕捉
  "薄盘紧钉扎=低量却高补单"的吸筹——但买侧吸筹本属弱信号，须配回测再定，避免放大噪声。

详见 [[signal-thresholds]]、[[known-bugs-and-gotchas]]。

---

## 迭代四十九：量级阈值 per-stock 校准（解耦 00100 标定，迈向通用化，2026-06-12）

### 动机

评估脚本通用性发现：架构早已多股就绪(`shared_config.STOCKS` + `--stock`)，但 monitor 顶部一批
**绝对量级阈值全是按 00100(~400 HKD 薄盘可做空)硬标定**的，换标的系统性失真：

- **HKD 绝对额**(14 个：`ICEBERG_MIN_NOTIONAL`、`SHORT_BLOCK_BIGFLOW_THRESHOLD`、
  `RETAIL_RETREAT_MIN_PEAK`、`DISTRIBUTION_*`、`MID_ACCUM_*` 等)：换大盘股(成交额 10–100×)
  被秒击穿=噪声；换低价小票=永不触发=失明。
- **股数**(2 个：`MF_THIN_DEPTH_SHARES=30,000`、`THIN_ASK_DEPTH_FOR_FLIP_SKIP=300`)：换厚盘股
  盘深永远 > 阈值 → 控盘/盘薄维度作废。

### 改动

**`shared_config.py`**：新增 `CALIBRATABLE: dict[str,str]` 注册表(16 个常量 → "flow"/"depth"
缩放类别)。每只股票可选配 `flow_scale`/`depth_scale`(默认 1.0=00100) + `calibration`(逐项显式覆盖)。
`00981` 加了占位示例(注释，未回测)。

**`short_squeeze_monitor.py`**：新增 `apply_stock_calibration(code)`——启动选股后，按缩放因子
就地改写本模块的量级阈值全局(`globals()` 在模块级函数返回模块命名空间，直接 mutate；所有阈值在
分析函数体内按调用时读取，启动期改写一次即全程生效)。白名单防止 config 错误 KEY 静默篡改无关全局。
入口在应用 stock_cfg 后调用并打印改动日志；非 00100 且未配缩放时 WARNING 提示可能失真。

**`probe-scale` 命令**(`cmd_probe_scale`)：盘中采样目标股 N 分钟，自动算建议缩放因子，免手猜倍数。
`python3 short_squeeze_monitor.py probe-scale --stock 00981 [--minutes 5]`：用 `get_market_snapshot`
累计 turnover 首尾差得每分钟成交额、`get_order_book` 卖盘十档深度取中位，对照 `shared_config.
CALIBRATION_BASELINE`(00100 参考量级)算 `flow_scale = 目标成交额/min ÷ 基准`、`depth_scale =
目标深度中位 ÷ 基准`，直接打印可粘贴的配置块。基准值是 00100 实盘估计，可用 `probe-scale --stock
00100` 重测校正。须交易时段运行(Δturnover≤0 会 WARNING)。已 mock 单测验证算式/输出/边界。

### 设计要点

- **单一真值源**：默认值仍是 monitor 顶部常量(保留各自注释)，`shared_config` 只存 per-stock 缩放/覆盖。
- **向后兼容**：00100 或未配置的股票 → `apply_stock_calibration` 返回空、零改动，行为与改造前完全一致
  (已单测验证)。
- **两个缩放因子覆盖 95%**：HKD 阈值随成交额同步放缩(flow)、股数随流通盘/股价放缩(depth)；
  个别项再用 `calibration` 精修。显式覆盖优先于缩放，负值阈值方向正确(均已单测)。
- **未覆盖范围**：比率/百分比类阈值(`ICEBERG_DOMINANCE`、失衡度、回落 %、`ICEBERG_PRICE_FLOOR`)
  本就跨标的可移植，不进 CALIBRATABLE。`paper_trader.py`/`short_position_manager.py` 若有自己的
  量级阈值，需同法另行处理(当前 paper_trader 读 DB 分数、不重算这些，无需改)。
- **⚠ 缩放因子须 `backtest_signals.py` 标定后再填**——跨标的预测力本就弱(迭代四十五 0/3)，
  缩放只是把"量级对齐"，不保证信号迁移有效。

详见 [[signal-thresholds]]、[[project-overview]]。

---

## 迭代五十：仪表盘新增日内 VWAP 线（机构执行基准 / 日内多空分水岭，2026-06-16）

### 动机
全天读盘缺一条**日内动态参照**：原仪表盘只有"空头成本 10/20 日加权线"（多日卖空成本，
`_vwap()` 算的是 Σ卖空额÷Σ卖空量，非股价 VWAP），没有当日成交量加权均价。机构盘中普遍以
VWAP 为执行基准（拆单/algo 锚定），价 > VWAP 偏强、价 < VWAP 偏弱，是判断日内多空与
机构吸/派的实用分水岭。

### 改动（`short_squeeze_monitor.py`）
- `MonitorState` 新增 `vwap: Optional[float]` 字段。
- 价格轮询块复用**已有的** `get_stock_quote` 返回（含 `volume`/`turnover` 两列，已查源码确认），
  直接 `vwap = turnover / volume`（当日累计），**无额外 API 调用**；带 0/NaN/异常守门，失败置 None。
- `print_dashboard` 头部"最新价"行扩展为 `最新价 X   日内VWAP Y (价±Z% · 上方偏强/下方偏弱)`；
  无数据时显示 `日内VWAP N/A`。

### 设计要点
- **零新增 API**：turnover/volume 本就在每轮 `get_stock_quote` 响应里，顺手算，无性能/限频代价。
- **只展示不计分**：VWAP 仅作读盘参照，未接入任何评分维度（避免引入未回测的新权重）。
- **与空头成本线区分**：VWAP=当日股价加权均价（日内）；空头成本线=多日卖空加权成本，二者并存不混。

### 回测铺垫：VWAP 穿越事件记录（log now, backtest later）
**背景**：`backtest_signals.py` 回放 `signals` 表事件，但 `price_history` 仅存 `(ts, price)` 无
volume/turnover，**历史 VWAP 无法重建**，且此前 VWAP 从不落库 → 现状无法回测。故先记录、攒数据。

- **`short_squeeze_monitor.py`**：价格轮询块新增 VWAP 穿越检测——`MonitorState._vwap_side`
  记上轮价相对 VWAP 位置（±1/None），带 `VWAP_CROSS_BAND=0.05%` 死区滤贴线微噪；明确上↔下切换时
  `db_save_signal` 记 `VWAP_CROSS_UP`(BULL) / `VWAP_CROSS_DOWN`(BEAR)，**score=0、不接入任何评分**。
  首次出带不算穿越（避免开盘伪信号）；已单测验证死区+切换逻辑。
- **`backtest_signals.py`**：`DIRECTION` 表登记两类（UP=BULL / DOWN=BEAR）。
- **验证路径**：积累约 10–15 交易日后 `python3 backtest_signals.py --type VWAP_CROSS_UP --rigor`
  看 NXT/EOD 的 exc 95%CI 是否在 0 一侧。验证有效再考虑接入评分；无效则保留为纯读盘参照。
- **已知小瑕疵**：`_vwap_side` 跨日不重置 → 至多漏记当日首个穿越，不产生伪事件（开盘价≈VWAP 落死区）。

详见 [[reading-playbook]]、[[project-overview]]、[[roadmap]]。

---

## 迭代五十一：量价确认维度（补"量"这个全脚本缺口，2026-06-17）

### 动机
脚本此前有价格、资金流、盘口、L2、VWAP，但**没有"量"维度**：无法区分 14:19 那种 46× 单笔猛砸
与随后 2–3× 的阴跌——本质是单笔/单位时间**成交量**差异，仪表盘看不出来。量价确认是机构行为
最基础的验证：真突破/真出货必带量，价创新高/新低却缩量=动能/抛压枯竭（背离），常为反转前兆。
（机构习惯增强清单 #1，见对话与 [[reading-playbook]]。）

### 改动
- **`price_history` 加 `volume` 列**（迁移 `_add_cols` 幂等，老库历史行补 NULL）。存**本轮增量量**
  （当日累计量差分，非累计）——是量价分析原语，也补上了迭代五十"VWAP 因无 volume 不可回测"的根因。
  `db_save_price(conn, ts, price, volume=None)` 改列名显式 INSERT。
- **`MonitorState`** 新增 `cum_volume` / `interval_volume` / `vol_surge_ratio` + 私有 `_prev_cum_volume` /
  `_vol_window`（近 N 轮增量量滚动窗，`field(default_factory=list)`）。
- **价格轮询块**：累计量差分得本轮增量 `_iv`；`vol_surge_ratio = _iv / median(近窗口)`（先算后入窗、
  不含本轮，攒够 `VOL_BASELINE_MIN_SAMPLES=12` 轮且中位>0 才出）；**累计量当前<上轮=跨日重置**清窗口。
- **仪表盘**：VWAP 行下方加 `本轮量 X 股 (Y× 近均量 · 放量/缩量/均量)`；`interval_volume` 为 None 时不显示。
- **常量**（近 `VWAP_CROSS_BAND`）：`VOL_BASELINE_WINDOW=40`、`VOL_BASELINE_MIN_SAMPLES=12`、
  `VOL_SURGE_MULT=1.5`、`VOL_DIVERGENCE_SHRINK=0.6`。

### 回测铺垫：量价背离事件记录（log now, backtest later）
- 价创日内新高(`>=session_high`)却 `surge_ratio<0.6` → `VOLPRICE_BEAR_DIV`(BEAR，动能枯竭)；
  创新低却缩量 → `VOLPRICE_BULL_DIV`(BULL，抛压枯竭)。**score=0、不接入评分**，默认 300s 冷却去重。
  `db_save_price` 先写入本轮再查 session_high/low（含本轮价），故 `_lp>=_hi` 即"本轮为日内极值"。
- **`backtest_signals.py`** `DIRECTION` 登记两类。攒约 10–15 日后 `--type VOLPRICE_BEAR_DIV --rigor`
  验证，有效再考虑接入评分。
- 已单测：首轮增量=None（无伪 surge）、放量/缩量倍数、跨日清窗口均符合预期。

### 设计要点
- **零新增 API**：volume/turnover 本就在每轮 `get_stock_quote` 响应内（VWAP 已在用）。
- **不依赖会冻结的 `get_capital_distribution`**：量价完全基于 quote，规避大单冻结陷阱（实盘见大单
  冻结 80+ 轮）。
- **延续"先记录后验证"纪律**：背离信号纯落库不计分，回测有效才接入。

详见 [[reading-playbook]]、[[project-overview]]、[[roadmap]]。

---

## 迭代五十二：尾盘机构动向（隔夜意图窗口，2026-06-17）

### 动机
机构习惯增强清单 #2。港股主板 16:00 收盘，最后 ~30min 机构常按**收盘价**执行（指数/被动盘锚 NAV、
algo 收尾），**尾盘资金净向 + 量能 + 收盘相对 VWAP 位置**往往泄露隔夜意图——比盘中嘈杂的挂单博弈
干净得多，是当日真实持仓变化的最佳窗口。

### 改动（`short_squeeze_monitor.py`）
- **`analyze_closing_behavior(conn, state, now)`**：进入尾盘窗口（`TAIL_SESSION_START=15:30`）后算三件事——
  ① 尾盘资金净向 = 最新累计 − 尾盘起锚点（`db_get_capital_at_or_after`）；② 尾盘量能比 =
  尾盘每轮均量 ÷ 当日早段每轮均量（`db_get_interval_vol_avg`，复用迭代五十一的 volume 列）；
  ③ 收盘相对 VWAP 位置。写入 `state.tail_*` 供仪表盘。
- **资金方向定性**：优先大单（若 |Δ|≥`TAIL_FLOW_MIN_DELTA`），否则中单——**大单冻结时中单当隐藏主力探针**
  （实盘大单常冻 80+ 轮）；量能完全基于 quote 不受冻结影响。
- **仪表盘**：尾盘窗口在 VWAP/量行下方加 `尾盘 净流入(大单) 大+400/中+20万 量1.6×放 VWAP上`（compact，56 列内）。
- 主循环 `print_dashboard` 前调用，传 `datetime.datetime.now()`。
- `MonitorState` 新增 `tail_active`/`tail_big_delta`/`tail_mid_delta`/`tail_vol_ratio`/`tail_bias` +
  `_closing_logged_date`（每日至多记一次事件）。
- 新 DB 读函数：`db_get_capital_at_or_after`（尾盘锚点）、`db_get_interval_vol_avg`（窗口均量）。

### 回测铺垫：收盘事件记录（log now, backtest later）
- 进最后 ~5min（`TAIL_EVENT_START=15:55`）+ VWAP 可用 + **方向与收盘位置共振**时记一次：
  资金净流入 + 收于 VWAP 上方 → `CLOSING_ACCUM`(BULL)；净流出 + 收于 VWAP 下方 → `CLOSING_DISTRIB`(BEAR)。
  **score=0 纯记录**，方向矛盾（如流入但价在 VWAP 下）则不记。`backtest_signals.py` `DIRECTION` 已登记两类。
- 攒约 10–15 日后 `--type CLOSING_ACCUM --rigor`（看次日开盘/隔夜收益）验证，有效再考虑接入评分。

### 设计要点
- **`TAIL_FLOW_MIN_DELTA` 已注册 `shared_config.CALIBRATABLE`(flow)**——HKD 绝对额，换标的随成交额缩放
  （迭代四十九纪律，否则大盘股秒触发/低价票永不触发）。其余阈值（时间窗、量能比、VWAP 位置）跨标的可移植。
- **零新增 API**：资金复用 capital_flow 表、量能复用 price_history.volume、VWAP 已在用。
- **延续"先记录后验证"纪律**：收盘事件纯落库不计分。
- 已单测：尾盘净向差分、量能比、共振判定、尾盘前不激活均符合预期。

详见 [[reading-playbook]]、[[project-overview]]、[[roadmap]]。

---

## 迭代五十三：冰山补单衰减 #4 + 挂单墙迁移 #5（机构习惯增强清单，2026-06-17）

两项都**不依赖会冻结的 `get_capital_distribution`**（纯 L2 逐笔 + 摆盘），延续"先记录后验证"纪律。

### #4 冰山补单衰减（反转前兆）
**动机**：连续同向冰山的**补单倍数递减**（实盘见过 53×→31×→15×）= 隐藏单即将枯竭，常是反转前兆；
原 `analyze_iceberg_absorption` 只记当前补单倍数、无衰减轨迹。
- `analyze_iceberg_absorption(conn, state=None)` 加可选 `state`；两个补单分支算出 `refill` 后调
  **`_record_iceberg_refill(conn, state, side, refill)`**（side +1 买侧吸筹 / -1 卖侧派发）。
- 维护 `MonitorState._ice_refill_hist`（末尾同向连续段）；连续 ≥`ICEBERG_EXHAUST_MIN_EVENTS=3` 次、
  起点 ≥`START_MULT=5.0`、逐次 ≤上次×`DECAY=0.6` → 记事件：**卖侧衰减→`ICEBERG_EXHAUST_SELL`(BULL，
  供给枯竭)**；**买侧衰减→`ICEBERG_EXHAUST_BUY`(BEAR，承接枯竭)**。`refill=inf`（best vol=0）不记录。
- 仪表盘经 `state.ice_exhaust_note` 显示 `冰山 卖侧补单衰减53→15×(供给枯竭)`（每轮重置）。

### #5 挂单墙迁移（主力意图）
**动机**：机构托价时随价**上移买墙**（守更高位=看多），派发时**下压卖墙**封顶（看空）——墙的迁移方向
比静态失衡更能看意图。
- `fetch_order_book` 扩展返回 `bid_wall_px/vol`、`ask_wall_px/vol`：**主导墙 = 十档内量最大且
  ≥ 其余档均值 × `WALL_DOMINANCE=2.5` 的档位**（纯相对比率，无绝对股数门槛，跨标的可移植）。
- **`analyze_wall_migration(conn, state, ob)`**（主循环 orderbook 块后调用）：维护
  `_bid_wall_hist`/`_ask_wall_hist`（墙缺失存 None 断连续段）；连续 `WALL_MIGRATE_ROUNDS=4` 轮单调 +
  净移动 ≥`WALL_MIGRATE_MIN_PCT=0.15%` → **买墙连升→`BID_WALL_LIFT`(BULL)**；**卖墙连降→`ASK_WALL_PRESS`(BEAR)**。
- 仪表盘经 `state.wall_migrate_note` 显示 `墙迁移 买墙↑441.0→442.8`。

### 公共
- `backtest_signals.py` `DIRECTION` 登记 4 类（EXHAUST_SELL/BUY、WALL_LIFT/PRESS）。攒约 10–15 日后
  `--type ICEBERG_EXHAUST_SELL --rigor` 等验证，有效再考虑接入评分。
- 全部**比率类阈值跨标的可移植，不入 CALIBRATABLE**（与迭代五十二的 HKD 绝对额 `TAIL_FLOW_MIN_DELTA` 不同）。
- 已单测：补单衰减触发/非陡降不触发/换向重置、墙主导判定、墙连升连降/含回撤不触发/微移<0.15%不触发。

详见 [[reading-playbook]]、[[project-overview]]、[[roadmap]]。

---

## 迭代五十四：算法单节奏 #3 + VWAP ±σ 通道 #8（机构习惯增强清单，2026-06-17）

两项均复用迭代五十一/五十的量/VWAP 原语，纯记录待验证，比率类阈值跨标的可移植（不入 CALIBRATABLE）。

### #3 算法单节奏 / TWAP 匀速
**动机**：TWAP 按时间均匀切片、POV 按市场量固定比例参与 → 单位时间成交量方差很低，区别于情绪/
脉冲盘；匀速 + 主动净方向 = 耐心机构吸筹/派发。
- **`analyze_algo_rhythm(conn, state)`**（主循环冰山后调用）：复用 `MonitorState._vol_window`（迭代五十一
  的逐轮增量量）算 CV=σ/均；CV ≤ `ALGO_TWAP_CV_MAX=0.40` 视为匀速。再用 `db_get_recent_ticks` 窗口主动
  买/卖净方向（≥ 另一方×`ALGO_RHYTHM_NET_DOM=1.3`）→ `ALGO_RHYTHM_BUY`(BULL，吸筹) / `ALGO_RHYTHM_SELL`
  (BEAR，派发)；匀速但无方向只出仪表盘提示不记事件。`state.vol_cv` / `state.algo_rhythm_note`（每轮重置）。
- 仪表盘：`节奏 匀速algo CV0.28 主动净买→吸筹`。需攒 `ALGO_RHYTHM_MIN_SAMPLES=16` 轮。

### #8 VWAP ±σ 通道
**动机**：日内价相对 VWAP 的偏离波动通道，均值回归参照。
- 价格轮询块（VWAP 已算处）维护 `_vwap_dev_window`（dev=价/VWAP−1）；攒够 `VWAP_BAND_MIN_SAMPLES=16` 轮
  算 `vwap_sigma=pstdev(dev)`，半宽=`VWAP_BAND_K=2.0`·σ。价 ≥+kσ → `VWAP_BAND_UPPER`(BEAR)、≤−kσ →
  `VWAP_BAND_LOWER`(BULL)、否则带内。`state.vwap_band_pos`。**趋势中价会沿轨运行（回归假设待回测证伪）**，
  故纯记录不计分。
- 仪表盘 VWAP 行下方：`VWAP带 ±0.57% (2.0σ) · 价在上轨`。

### 公共
- `backtest_signals.py` `DIRECTION` 登记 4 类。攒约 10–15 日后 `--type ALGO_RHYTHM_SELL --rigor` 等验证。
- 已单测：CV 匀速/脉冲判别、净方向、band 上轨/下轨/带内、σ 样本不足不触发。

至此机构习惯增强清单 #1/#2/#3/#4/#5/#8 全部落地（均 log-only 待验证）；**剩 #6 对倒/席位轮动**（用
broker_queue，价值高且干净，下一步）、**#7 撤单率/幌骗量化**（L1 数据噪声大、价值存疑，建议回测前 4 项
有效后再评估是否值得做）。
...
## 迭代五十五：强势多头态屏蔽卖侧冰山派发分（反指闸门，2026-06-18）

**动机**：实盘 00100（MINIMAX-W）2026-06-18 早盘，价守 VWAP 上方 +2.5~4%、中单(拆单)累计 +1,400 万
持续吸筹、Failsafe 4 全程「禁止开空」。但 `analyze_iceberg_absorption` 的卖侧冰山派发（主动买主导 +
价滞涨 + 卖一补单 44~75×）持续命中，向做空注入 +8/+15 背景分，且 30 秒内卖侧派发↔买侧吸筹反复翻转，
把做空分从 8 抖到 31 又砸回 0——纯横跳噪音。根因：单边多头控盘票里「卖一不断补单」多为主力控盘卖墙
（控节奏/洗浮筹），并非真出货，派发逻辑在此沦为**反指**（延续 2026-06-10 [[case-00100-20260610-false-short]]
教训：冰山派发=反指、中单吸筹才是真信号）。

**改动（`short_squeeze_monitor.py`）**：
- 新增 `bullish_accum_active(conn, current_price, big_net_stale)`：强势多头吸筹态布尔判定，条件**刻意复用
  Failsafe 4 弱档**（`big_net_stale` + 中单 `accel`(Δ≥`MID_ACCUM_MIN_DELTA`) 或 `level`(累计≥`MID_ACCUM_MIN_LEVEL`)
  + `price_rise_pct≥MID_ACCUM_PRICE_RISE_PCT`），保证「禁止开空」与「屏蔽派发分」同步、不矛盾。
- 主循环 `analyze_iceberg_absorption` 之后、`analyze_broker_footprint` 之前加闸：`s_ice_sup>0` 且命中该态
  → `s_ice_sup, sg_ice = 0, []` 并记 `[冰山派发屏蔽]` 日志。位置在 broker 之前，连带掐掉派发的经纪席位
  加成 `s_brk_sup`；在 `state.latest_ice_sup` 赋值之前，仪表盘 L2 行同步清掉「卖侧派发」。

**设计要点**：只屏蔽**卖侧派发支撑分**，买侧冰山吸筹（抬升逼空）与逼空侧不受影响；非多头态（含价格
下行的合法做空）`bullish_accum_active` 天然返回 False（price_rise_pct<0.2），不伤害晨盘做空。HKD 阈值随
标的量级，换低价/小盘股需随 Failsafe 4 一起重标定。

详见 [[case-00100-20260610-false-short]]、[[capital-flow-tiers-intent]]、[[known-bugs-and-gotchas]]。

---

## 迭代五十六：派发确认价格证伪闸（Bug 修复，2026-06-18）

**Bug（实盘 00100 2026-06-18 15:17~15:22）**：尾盘三档资金净流出触发派发模式确认，做空=83~100 ENTRY
连续刷屏，但价格从日内低 469.4 **反弹 +2% 收复 VWAP 打到上轨 479**——做空全程诱空，盘口同时打印
**买侧冰山吸筹（买一补单 370×）+ JPM/荷银经纪吸筹足迹 + 逼空分升至 42**，全被无视。

**根因**：`analyze_distribution_mode` 三条件全锚定**日内极值 + 冻结累计值**：① `big_at_low`(big_now≤
`big_low`×ratio)、② 散户撤(`small_peak` 峰值回落%)、③ 中小单撤(`mid_small_peak` 峰值回落%)。大单冻结
在 -2,087 万(=日内低附近)时三条件**恒真且无价格证伪出口**，`distribution_confirmed` 沦为**单向锁存到
收盘**。安全门通道 B（`channel_b=distribution_confirmed`，[L3148]）无条件放行，且 `s_dist`+25 / sticky
+12 每轮重算把做空硬推到 100。对比通道 C 在迭代二十六已加 `still_at_lows` 反弹护栏，**通道 B 始终缺这
道护栏**——即不对称缺陷。

**改动（`short_squeeze_monitor.py`，主循环 4342 调用处）**：在 sticky 块之前加价格证伪闸——
`(dist_confirmed 或 sticky>0)` 且价收复 VWAP 上方（`(last_price-vwap)/vwap > VWAP_CROSS_BAND`，即仪表盘
「上方偏强」）→ `s_dist, dist_confirmed, sg_dist = 0, False, []` 且 `_distribution_sticky_left = 0`，记
`[派发证伪]` 日志。一处作废**三条传导**：+25 主信号分、通道 B 放行、翻转 failsafe 豁免（`distribution_active`）。

**设计要点**：阈值复用 `VWAP_CROSS_BAND=0.05%`，与仪表盘 上方偏强/下方偏弱 同源，语义=「派发确认仅在
价不强于 VWAP 时有效」。原版 2026-05-29 真派发案例价是**下行**（VWAP 下方，dev<0），不触发证伪、行为
不变。与迭代五十五（多头态屏蔽假冰山派发）对称：五十五治「假派发压多头」，五十六治「过期派发压住
该否决做空的实时吸筹信号」。

详见 [[case-00100-20260610-false-short]]、[[known-bugs-and-gotchas]]、[[signal-thresholds]]。
