# MINIMAX-W (00100.HK) 融券做空套利系统

港股 MINIMAX-W 融券池与做空数据套利工具，面向**富途普通账户**。  
...
| 文件 | 说明 |
|------|------|
| `short_data.db` | 主数据库（HKEX 日线、摆盘快照、资金流向、信号记录） |
| `short_position.json` | 持仓状态快照，程序重启后自动恢复 |
| `short_monitor.log` | 监控器日志 |
| `position_manager.log` | 持仓管理器日志 |
