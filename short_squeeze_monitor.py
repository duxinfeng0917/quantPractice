...
from shared_config import (HKEX_COST_WINDOW, STOCKS, DEFAULT_STOCK,
                           CALIBRATABLE, CALIBRATION_BASELINE)
...
# 价格闸标定（迭代四十八，2026-06-12）：原 ±0.10% 对高价票结构性偏紧——00100@407 时
# 0.10%≈0.4 HKD≈2 个 tick(0.2)，4 轮窗口内晃几跳就破闸，把"价守住的被动吸筹"误判成"在跌→真卖压"
# （实盘 10:55 段主卖 4×、价 406→412 横盘前，窗口 -0.25% 被挡，冰山全程未报）。
# 放宽至 ±0.25%(@407≈5 ticks/≈1.0 HKD，仍属"基本守位")。注：买侧吸筹分已按迭代四十五降权
# 至 8/4(回测看多预测力弱)，故此处只修"价守位"判定口径，不动分值，避免放大弱信号。
# 日内 VWAP 穿越事件死区：|价−VWAP|/VWAP 超过此比例才算明确在上/下方，滤价格贴线时的微噪反复
# （穿越事件仅写入 signals 表供 backtest_signals 验证，不接入任何评分）
VWAP_CROSS_BAND         = 0.0005     # 0.05%
# ── VWAP ±σ 通道（迭代五十四 #8）：日内价相对 VWAP 的偏离波动通道，均值回归参照 ──
# dev=价/VWAP−1 的滚动 σ；价 ≥ +k·σ=上轨（偏离过度，回归向下偏空）/ ≤ −k·σ=下轨（回归向上偏多）。
# 触轨事件仅记录供 backtest 验证（趋势中价会沿轨运行，回归假设待证），不接入评分。
VWAP_BAND_WINDOW         = 40        # dev 样本滚动窗口（轮数）
VWAP_BAND_MIN_SAMPLES    = 16        # 攒够这么多轮才算 σ / 判触轨
VWAP_BAND_K              = 2.0       # 通道半宽 = k·σ
# ── 算法单节奏 / TWAP 匀速（迭代五十四 #3）：逐轮成交量低离散 = 耐心机构匀速吃/出 ──
# 复用 _vol_window；CV=σ/均 ≤ 阈值视为匀速节奏，结合主动净方向→吸筹/派发（事件纯记录不计分）。
ALGO_RHYTHM_MIN_SAMPLES  = 16        # 至少这么多轮 interval volume 才判节奏
ALGO_TWAP_CV_MAX         = 0.40      # 逐轮量 CV ≤ 此值 → 匀速（TWAP/POV 足迹）
ALGO_RHYTHM_NET_DOM      = 1.3       # 窗口主动买/卖 ≥ 另一方×此倍 才算有方向
# ── 量价确认（迭代五十一）：本轮增量成交量(累计差分) vs 近窗口中位量 ──
# 量价背离事件仅写 signals 表供 backtest 验证，不接入任何评分；仪表盘展示均量倍数。
VOL_BASELINE_WINDOW      = 40        # 量能基准滚动窗口（轮数，~10min @15s）
VOL_BASELINE_MIN_SAMPLES = 12        # 攒够这么多轮才出均量倍数/背离判定
VOL_SURGE_MULT           = 1.5       # 本轮量 ≥ 基准×此值 → 放量
VOL_DIVERGENCE_SHRINK    = 0.6       # 创新高/新低时本轮量 < 基准×此值 → 缩量背离
# ── 尾盘机构动向（迭代五十二）：港股主板 16:00 收盘，最后 ~30min 机构常按收盘价执行 ──
# （指数/被动盘锚 NAV、algo 收尾），尾盘资金净向 + 量能 + 收盘相对 VWAP 位置泄露隔夜意图。
TAIL_SESSION_START       = (15, 30)  # 尾盘动向监测起点 (时,分)，仪表盘从此刻起显示尾盘行
TAIL_EVENT_START         = (15, 55)  # 收盘事件记录起点（最后 ~5min；全程刷事件无意义）
TAIL_FLOW_MIN_DELTA      = 2_000_000 # 尾盘大单/中单净变化 ≥ 此额(HKD) 才算明显方向（calibratable·flow）
TAIL_VOLUME_SURGE        = 1.3       # 尾盘每轮均量 / 全日每轮均量 ≥ 此值 → 尾盘放量
ICEBERG_PRICE_FLOOR     = -0.25      # 吸筹：同期价格涨幅 ≥ 此 %（未跟跌）
ICEBERG_PRICE_CAP       = 0.25       # 派发：同期价格涨幅 ≤ 此 %（滞涨）
...
# ── 冰山补单衰减（迭代五十三 #4）：连续同向冰山的补单倍数递减 = 隐藏单枯竭 → 反转前兆 ──
# 仅记录事件供 backtest 验证，不接入评分。比率类阈值跨标的可移植，不入 CALIBRATABLE。
ICEBERG_EXHAUST_MIN_EVENTS = 3       # 连续 ≥ 此次同向冰山才评估衰减
ICEBERG_EXHAUST_START_MULT = 5.0     # 衰减段起点补单倍数 ≥ 此值（曾是强补单）才算"枯竭"
ICEBERG_EXHAUST_DECAY      = 0.6     # 每次补单倍数 ≤ 上次 × 此值 → 明确递减
...
# ── 挂单墙迁移（迭代五十三 #5）：主导挂单墙逐轮上/下移 = 主力意图（抬承接/压封顶）──
# 墙 = 十档内成交量最大且 ≥ 其余档均值 × WALL_DOMINANCE 的档位（纯相对比率，跨标的可移植）。
# 连升/连降事件仅记录供 backtest 验证，不接入评分。
WALL_DOMINANCE          = 2.5        # 主导墙量 ≥ 其余档均值 × 此倍数才算"墙"
WALL_MIGRATE_ROUNDS     = 4          # 连续 N 轮墙价单调移动才算迁移（~1min）
WALL_MIGRATE_MIN_PCT    = 0.0015     # 窗口内墙价净移动 ≥ 此比例（0.15%）滤微噪
...
# ── 对倒/席位轮动（迭代五十四-b #6）：同一机构席位反复同时占据买一+卖一 = 疑似对倒做量 ──
# 仅作"成交量打折看"的上下文警示（非多空预测），事件 direction=None(中)。计数阈值跨标的可移植。
# 零售聚合商(富途/老虎)天然两侧皆有(汇集散户买卖)，不算对倒，故只看机构席位（见 fetch_broker_queue）。
WASH_WINDOW                 = 8   # 回看 broker_queue 轮数
WASH_MIN_ROUNDS             = 5   # 同一机构同席双挂出现 ≥ 此轮数 → 疑似对倒
...
def init_db(db_path: str) -> sqlite3.Connection:
    ...
    # 迭代五十一 量价确认：price_history 增量成交量列（老库历史行补 NULL）。
    # 存"本轮增量量"(累计差分)，非累计——是量价背离/放量确认的分析原语，供 backtest 用。
    _add_cols("price_history", {"volume": "REAL"})
    conn.commit()
    return conn
...
def db_get_capital_at_or_after(
    conn: sqlite3.Connection, ts: str,
) -> Optional[tuple[float, float, float]]:
    """返回 ts(含) 之后最早一条 (big_net, mid_net, small_net)；无则 None。

    尾盘动向锚点用（迭代五十二）：取尾盘窗口起点的资金累计值，与最新累计相减 = 尾盘净变化。
    """
    row = conn.execute(
        "SELECT big_net, mid_net, small_net FROM capital_flow "
...
def db_get_interval_vol_avg(
    conn: sqlite3.Connection, since_ts: str, until_ts: Optional[str] = None,
) -> Optional[float]:
    """price_history.volume（本轮增量量）在 [since_ts, until_ts) 的每轮均值（仅正值）。

    尾盘量能比用（迭代五十二）：尾盘窗口均量 vs 当日早段均量。无样本返回 None。
    """
    q = ("SELECT AVG(volume) FROM price_history "
         "WHERE ts >= ? AND volume IS NOT NULL AND volume > 0")
...
def db_save_price(conn: sqlite3.Connection, ts: str, price: float,
    ...
    if price is None or not math.isfinite(price) or price <= 0:
        ...
        return
    # volume = 本轮增量成交量（累计差分，股）；首轮/跨日不可得时存 NULL（迭代五十一）
    conn.execute(
        "INSERT INTO price_history (ts, price, volume) VALUES (?,?,?)",
        (ts, price, volume),
    )
    conn.commit()
...
# ═══════════════════════════════════════════════════════════
def _record_iceberg_refill(
    conn: sqlite3.Connection, state: Optional["MonitorState"],
    side: int, refill: float,
) -> None:
    """记录本次冰山补单倍数并检测连续同向衰减（迭代五十三 #4）。

    连续 ≥ICEBERG_EXHAUST_MIN_EVENTS 次同向冰山、起点 ≥START_MULT、逐次 ≤上次×DECAY →
    隐藏单枯竭，记 ICEBERG_EXHAUST_* 事件（score=0 纯记录，供 backtest 验证；db_save_signal 自带去重）：
      - side<0 卖侧派发补单衰减 → 上方供给枯竭 → 看多（ICEBERG_EXHAUST_SELL=BULL）
      - side>0 买侧吸筹补单衰减 → 下方承接枯竭 → 看空（ICEBERG_EXHAUST_BUY=BEAR）
    """
    if state is None or not math.isfinite(refill):
        return
    state._ice_refill_hist.append((side, refill))
    if len(state._ice_refill_hist) > 6:
        state._ice_refill_hist.pop(0)
    run: list[float] = []                       # 末尾同向连续段，旧→新
    for s, r in reversed(state._ice_refill_hist):
        if s == side:
            run.append(r)
        else:
            break
    run.reverse()
    if (len(run) >= ICEBERG_EXHAUST_MIN_EVENTS and run[0] >= ICEBERG_EXHAUST_START_MULT
            and all(run[i + 1] <= run[i] * ICEBERG_EXHAUST_DECAY for i in range(len(run) - 1))):
        if side < 0:
            db_save_signal(conn, "ICEBERG_EXHAUST_SELL",
                f"卖侧冰山补单衰减 {run[0]:.0f}×→{run[-1]:.1f}×（派发供给枯竭，上方压力减轻）", 0)
            state.ice_exhaust_note = f"卖侧补单衰减{run[0]:.0f}→{run[-1]:.1f}×(供给枯竭)"
        else:
            db_save_signal(conn, "ICEBERG_EXHAUST_BUY",
                f"买侧冰山补单衰减 {run[0]:.0f}×→{run[-1]:.1f}×（吸筹承接枯竭，下方支撑减弱）", 0)
            state.ice_exhaust_note = f"买侧补单衰减{run[0]:.0f}→{run[-1]:.1f}×(承接枯竭)"


def analyze_iceberg_absorption(
    conn: sqlite3.Connection, state: Optional["MonitorState"] = None,
    ...
    传入 state 时附带记录补单倍数并检测衰减（迭代五十三 #4，纯事件不改本函数返回的评分）。
    返回 (squeeze_pts, support_pts, signals)：二者按价格方向互斥，单次最多一个非零。
    """
    if state is not None:
        state.ice_exhaust_note = None           # 每轮重置，仅本轮衰减命中时置回
    ...
    if (sell_notional >= ICEBERG_MIN_NOTIONAL and sell_tot >= buy_tot * ICEBERG_DOMINANCE
        ...
        _record_iceberg_refill(conn, state, +1, refill)   # 买侧吸筹（迭代五十三 #4）
    ...
    if (buy_notional >= ICEBERG_MIN_NOTIONAL and buy_tot >= sell_tot * ICEBERG_DOMINANCE
        ...
        _record_iceberg_refill(conn, state, -1, refill)   # 卖侧派发（迭代五十三 #4）
        if refill >= ICEBERG_REFILL_MULT:
...
def bullish_accum_active(
    conn: sqlite3.Connection, current_price: Optional[float], big_net_stale: bool
) -> bool:
    """强势多头吸筹态检测（与 Failsafe 4 弱档同源）：大单冻结期中单(拆单)加速/累积
    净买入 + 同期价格上行。命中时盘口偏空的「卖侧冰山派发」多为主力控盘卖墙而非真出货，
    在 00100 类单边多头票上反复横跳、30 秒内卖侧↔买侧翻转，沦为做空反指
    （2026-06-18 实盘 00100：派发 +15/+8 持续向做空注入背景分制造横跳）。据此屏蔽其
    支撑做空分。条件刻意复用 Failsafe 4 弱档（accel 或 level + price_rise≥0.2%），
    保证「禁止开空」与「屏蔽冰山派发」同步生效、不互相矛盾。"""
    if not big_net_stale or current_price is None:
        return False
    cap = db_get_recent_capital_structure(conn, MID_ACCUM_WINDOW)
    prices = db_get_recent_prices(conn, MID_ACCUM_PRICE_WINDOW)
    if len(cap) < 3 or len(prices) < 3 or prices[0] <= 0:
        return False
    mid_latest = cap[0][1]
    mid_delta  = cap[0][1] - cap[-1][1]
    price_rise_pct = (current_price - prices[0]) / prices[0] * 100
    accel = mid_delta >= MID_ACCUM_MIN_DELTA
    level = mid_latest >= MID_ACCUM_MIN_LEVEL
    return mid_latest > 0 and (accel or level) and price_rise_pct >= MID_ACCUM_PRICE_RISE_PCT


# ═══════════════════════════════════════════════════════════
# 五-D3b、挂单墙迁移（迭代五十三 #5）：主导墙逐轮上/下移 = 主力意图
# ═══════════════════════════════════════════════════════════
def analyze_wall_migration(
    conn: sqlite3.Connection, state: MonitorState, ob: Optional[dict],
) -> None:
    """主导挂单墙（十档内最大量档）逐轮单调迁移 = 主力抬承接位 / 压封顶位。

    机构习惯：托价时随价上移买墙（守更高位=看多），派发时下压卖墙封顶（看空）。
    连续 WALL_MIGRATE_ROUNDS 轮墙价单调且净移动 ≥WALL_MIGRATE_MIN_PCT → 记事件（score=0 纯记录，
    供 backtest 验证；db_save_signal 自带去重，不接入评分）：
      - 买墙连升 → BID_WALL_LIFT (BULL)；卖墙连降 → ASK_WALL_PRESS (BEAR)。
    墙缺失轮存 None，断开连续段。仪表盘经 state.wall_migrate_note 展示。
    """
    state.wall_migrate_note = None
    if not ob:
        return
    state.bid_wall_px = ob.get("bid_wall_px")
    state.ask_wall_px = ob.get("ask_wall_px")
    state._bid_wall_hist.append(state.bid_wall_px)
    state._ask_wall_hist.append(state.ask_wall_px)
    if len(state._bid_wall_hist) > WALL_MIGRATE_ROUNDS:
        state._bid_wall_hist.pop(0)
    if len(state._ask_wall_hist) > WALL_MIGRATE_ROUNDS:
        state._ask_wall_hist.pop(0)

    notes: list[str] = []
    bh = state._bid_wall_hist
    if (len(bh) == WALL_MIGRATE_ROUNDS and all(x is not None and x > 0 for x in bh)
            and all(bh[i] < bh[i + 1] for i in range(len(bh) - 1))
            and (bh[-1] - bh[0]) / bh[0] >= WALL_MIGRATE_MIN_PCT):
        notes.append(f"买墙↑{bh[0]:.2f}→{bh[-1]:.2f}")
        db_save_signal(conn, "BID_WALL_LIFT",
            f"买墙连升 {bh[0]:.2f}→{bh[-1]:.2f}（主力抬高承接位，托价意图）", 0)
    ah = state._ask_wall_hist
    if (len(ah) == WALL_MIGRATE_ROUNDS and all(x is not None and x > 0 for x in ah)
            and all(ah[i] > ah[i + 1] for i in range(len(ah) - 1))
            and (ah[0] - ah[-1]) / ah[0] >= WALL_MIGRATE_MIN_PCT):
        notes.append(f"卖墙↓{ah[0]:.2f}→{ah[-1]:.2f}")
        db_save_signal(conn, "ASK_WALL_PRESS",
            f"卖墙连降 {ah[0]:.2f}→{ah[-1]:.2f}（主力压低封顶位，派发意图）", 0)
    if notes:
        state.wall_migrate_note = " · ".join(notes)


# ═══════════════════════════════════════════════════════════
# 五-D3c、算法单节奏 / TWAP 匀速（迭代五十四 #3）
# ═══════════════════════════════════════════════════════════
def analyze_algo_rhythm(conn: sqlite3.Connection, state: MonitorState) -> None:
    """逐轮 interval volume 低离散（CV ≤ ALGO_TWAP_CV_MAX）= 耐心机构匀速吃/出（TWAP/POV 足迹）。

    机构习惯：TWAP 按时间均匀切片、POV 按市场量固定比例参与 → 单位时间成交量方差很低，
    区别于情绪/脉冲盘。匀速成立时再结合窗口主动净方向 → 匀速吸筹(BULL)/匀速派发(BEAR)。
    事件 score=0 纯记录供 backtest 验证，不接入评分；仪表盘经 state.algo_rhythm_note 展示。
    """
    state.algo_rhythm_note = None
    state.vol_cv = None
    win = [v for v in state._vol_window if v is not None]
    if len(win) < ALGO_RHYTHM_MIN_SAMPLES:
        return
    mean = sum(win) / len(win)
    if mean <= 0:
        return
    cv = statistics.pstdev(win) / mean
    state.vol_cv = cv
    if cv > ALGO_TWAP_CV_MAX:
        return                                  # 离散高 → 非匀速（情绪/脉冲盘）
    rows = db_get_recent_ticks(conn, ICEBERG_WINDOW)
    sell_tot = sum(r[0] for r in rows)
    buy_tot  = sum(r[1] for r in rows)
    if buy_tot >= sell_tot * ALGO_RHYTHM_NET_DOM and buy_tot > 0:
        state.algo_rhythm_note = f"匀速algo CV{cv:.2f} 主动净买→吸筹"
        db_save_signal(conn, "ALGO_RHYTHM_BUY",
            f"逐轮量匀速(CV{cv:.2f})+主动净买 {buy_tot:,.0f}>卖 {sell_tot:,.0f} → 耐心机构吸筹", 0)
    elif sell_tot >= buy_tot * ALGO_RHYTHM_NET_DOM and sell_tot > 0:
        state.algo_rhythm_note = f"匀速algo CV{cv:.2f} 主动净卖→派发"
        db_save_signal(conn, "ALGO_RHYTHM_SELL",
            f"逐轮量匀速(CV{cv:.2f})+主动净卖 {sell_tot:,.0f}>买 {buy_tot:,.0f} → 耐心机构派发", 0)
    else:
        state.algo_rhythm_note = f"匀速algo CV{cv:.2f}（无明显方向）"
...
def fetch_order_book(ctx: OpenQuoteContext, conn: sqlite3.Connection
    ...
    bid_prices = [float(item[0]) for item in bid_list]
    ask_prices = [float(item[0]) for item in ask_list]

    # 主导挂单墙（迭代五十三 #5）：十档内量最大且 ≥ 其余档均值 × WALL_DOMINANCE 的档位价。
    # 纯相对比率判定（无绝对股数门槛），跨标的可移植；无明显墙时返回 (None, 0)。
    def _wall(prices: list, vols: list) -> tuple[Optional[float], float]:
        if not vols:
            return None, 0.0
        i = max(range(len(vols)), key=lambda k: vols[k])
        others = [v for k, v in enumerate(vols) if k != i]
        avg_other = (sum(others) / len(others)) if others else 0.0
        if vols[i] > 0 and (avg_other <= 0 or vols[i] >= avg_other * WALL_DOMINANCE):
            return prices[i], vols[i]
        return None, 0.0
    bid_wall_px, bid_wall_vol = _wall(bid_prices, bid_vols)
    ask_wall_px, ask_wall_vol = _wall(ask_prices, ask_vols)
    ...
    return {"ts": ts, "bid_depth": bid_depth,
            ...
            "bid_wall_px": bid_wall_px, "bid_wall_vol": bid_wall_vol,
            "ask_wall_px": ask_wall_px, "ask_wall_vol": ask_wall_vol}
...
def fetch_broker_queue(ctx: OpenQuoteContext, conn: sqlite3.Connection
    ...
    # 同席双挂（对倒指纹，迭代五十四-b #6）：同一**机构**席位同时现于买一+卖一。零售聚合商
    # (富途/老虎)天然两侧都有（汇集散户买卖），不算对倒，故只看机构席位；取重叠最强者。
    def _cross_inst(bc: dict, ac: dict) -> tuple[Optional[str], int]:
        best_nm, best_ov = None, 0
        for nm, c in bc.items():
            if not _broker_is_institutional(nm):
                continue
            ov = min(c, ac.get(nm, 0))
            if ov > best_ov:
                best_nm, best_ov = nm, ov
        return best_nm, best_ov
    cross_name, cross_overlap = _cross_inst(bid_cnt, ask_cnt)
    ...
    return {"ts": ts, "bid_top_name": bid_top_name, "bid_top_net": bid_top_net,
            ...
            "cross_name": cross_name, "cross_overlap": cross_overlap}
...
class MonitorState:
    ...
    vwap:              Optional[float] = None   # 日内成交量加权均价 = 当日累计成交额 / 累计成交量
    _vwap_side:        Optional[int]   = None   # 上轮价相对 VWAP 的位置：+1 上方/-1 下方/None 带内（穿越检测用）
    # 量价确认（迭代五十一）：本轮增量量 = 当日累计量差分；均量倍数 = 本轮量 ÷ 近窗口中位量
    cum_volume:        Optional[float] = None   # 当日累计成交量（最新一轮）
    interval_volume:   Optional[float] = None   # 本轮增量成交量（股），首轮/跨日为 None
    vol_surge_ratio:   Optional[float] = None   # 本轮量 / 近窗口中位量（攒够样本才出）
    _prev_cum_volume:  Optional[float] = None
    _vol_window:       list[float]     = field(default_factory=list)  # 近 N 轮 interval volume（最旧在前）
    # VWAP ±σ 通道（迭代五十四 #8）
    _vwap_dev_window:  list[float]     = field(default_factory=list)  # 近 N 轮 价/VWAP−1 偏离
    vwap_sigma:        Optional[float] = None   # 偏离的滚动标准差
    vwap_band_pos:     Optional[str]   = None   # 价位于 上轨/下轨/带内
    # 算法单节奏 / TWAP（迭代五十四 #3）
    vol_cv:            Optional[float] = None   # 逐轮 interval volume 离散系数
    algo_rhythm_note:  Optional[str]   = None   # 匀速节奏提示（仪表盘，每轮重置）
    # 对倒/席位轮动（迭代五十四-b #6）
    _wash_hist:        list            = field(default_factory=list)  # 近 N 轮同席双挂机构名（None=无）
    wash_note:         Optional[str]   = None   # 对倒疑似提示（仪表盘，每轮重置）
    # 尾盘机构动向（迭代五十二）：仅在尾盘窗口（15:30+）计算
    tail_active:       bool            = False  # 当前是否处于尾盘窗口（仪表盘据此显示尾盘行）
    tail_big_delta:    Optional[float] = None   # 尾盘起至今 大单净变化（HKD）
    tail_mid_delta:    Optional[float] = None   # 尾盘起至今 中单净变化（HKD）
    tail_vol_ratio:    Optional[float] = None   # 尾盘每轮均量 / 全日每轮均量
    tail_bias:         Optional[str]   = None   # 尾盘动向定性文字（仪表盘展示）
    _closing_logged_date: Optional[str] = None  # 收盘事件已记录日期（每日至多一次）
    ...
    # 冰山补单衰减（迭代五十三 #4）：(side, refill) 历史，side +1 买侧吸筹/-1 卖侧派发
    _ice_refill_hist: list = field(default_factory=list)
    ice_exhaust_note: Optional[str] = None  # 本轮冰山补单衰减提示（仪表盘，每轮重置）
    # 挂单墙迁移（迭代五十三 #5）：主导买/卖墙价历史 + 当前墙价 + 迁移提示
    bid_wall_px:   Optional[float] = None
    ask_wall_px:   Optional[float] = None
    _bid_wall_hist: list = field(default_factory=list)
    _ask_wall_hist: list = field(default_factory=list)
    wall_migrate_note: Optional[str] = None
...
def analyze_closing_behavior(
    conn: sqlite3.Connection, state: MonitorState, now: datetime.datetime,
) -> None:
    """尾盘机构动向（迭代五十二）：进入尾盘窗口（15:30+）后算 尾盘资金净向 + 量能 + 收盘相对
    VWAP 位置，写入 state 供仪表盘展示；进入最后 ~5min（15:55+）方向与收盘位置共振时记一次
    CLOSING_ACCUM/CLOSING_DISTRIB 事件（score=0 纯记录，供 backtest 验证）。

    机构习惯：指数/被动盘按收盘价执行锚 NAV、algo 尾盘收尾——尾盘资金净向往往泄露隔夜意图。
    资金维度遇大单冻结优先用中单（实盘大单常冻 80+ 轮）；量能完全基于 quote 不受冻结影响。
    """
    t = now.time()
    if t < datetime.time(*TAIL_SESSION_START):
        state.tail_active = False
        state.tail_bias = None
        return
    state.tail_active = True
    today = now.date().isoformat()
    tail_start_ts = datetime.datetime.combine(
        now.date(), datetime.time(*TAIL_SESSION_START)
    ).isoformat(timespec="seconds")

    # ① 尾盘资金净向：当前累计 − 尾盘起锚点
    anchor = db_get_capital_at_or_after(conn, tail_start_ts)
    if (anchor is not None and state.latest_big_net is not None
            and state.latest_mid_net is not None):
        state.tail_big_delta = state.latest_big_net - anchor[0]
        state.tail_mid_delta = state.latest_mid_net - anchor[1]
    else:
        state.tail_big_delta = state.tail_mid_delta = None

    # ② 尾盘量能比：尾盘窗口每轮均量 / 当日早段每轮均量
    tail_avg = db_get_interval_vol_avg(conn, tail_start_ts)
    day_avg  = db_get_interval_vol_avg(conn, today, tail_start_ts)
    state.tail_vol_ratio = (tail_avg / day_avg) if (tail_avg and day_avg and day_avg > 0) else None

    # ③ 资金方向定性：优先大单（若显著），否则中单（大单冻结时的隐藏主力探针）
    bd, md = state.tail_big_delta, state.tail_mid_delta
    flow_dir, flow_src = 0, ""
    if bd is not None and abs(bd) >= TAIL_FLOW_MIN_DELTA:
        flow_dir, flow_src = (1 if bd > 0 else -1), "大单"
    elif md is not None and abs(md) >= TAIL_FLOW_MIN_DELTA:
        flow_dir, flow_src = (1 if md > 0 else -1), "中单"

    # ④ 收盘相对 VWAP 位置
    above_vwap = bool(state.last_price is not None and state.vwap
                      and state.last_price >= state.vwap)

    # 仪表盘定性文字（compact，_box 56 列内）
    seg = [{1: "净流入", -1: "净流出", 0: "资金平淡"}[flow_dir]
           + (f"({flow_src})" if flow_src else "")]
    fl = []
    if bd is not None: fl.append(f"大{bd/10000:+.0f}")
    if md is not None: fl.append(f"中{md/10000:+.0f}")
    if fl: seg.append("/".join(fl) + "万")
    if state.tail_vol_ratio is not None:
        seg.append(f"量{state.tail_vol_ratio:.1f}×"
                   + ("放" if state.tail_vol_ratio >= TAIL_VOLUME_SURGE else "缩"))
    if state.vwap and state.last_price:
        seg.append("VWAP上" if above_vwap else "VWAP下")
    state.tail_bias = " ".join(seg)

    # ⑤ 收盘事件记录（每日至多一次，进最后 ~5min 后；需 VWAP 可用 + 方向与收盘位置共振）
    if (t >= datetime.time(*TAIL_EVENT_START)
            and state._closing_logged_date != today and state.vwap):
        if flow_dir > 0 and above_vwap:
            db_save_signal(conn, "CLOSING_ACCUM",
                f"尾盘机构吸筹收尾：{flow_src}净流入 + 收于VWAP上方（{state.tail_bias}）", 0)
            state._closing_logged_date = today
        elif flow_dir < 0 and not above_vwap:
            db_save_signal(conn, "CLOSING_DISTRIB",
                f"尾盘机构派发收尾：{flow_src}净流出 + 收于VWAP下方（{state.tail_bias}）", 0)
            state._closing_logged_date = today
...
def print_dashboard(
    ...
    print(f"╠{bar58}╣")
    # 日内 VWAP：成交量加权均价（机构执行基准 / 日内多空分水岭）
    if state.vwap and state.last_price:
        _dev = (state.last_price - state.vwap) / state.vwap * 100
        _side = "上方偏强" if _dev > 0 else ("下方偏弱" if _dev < 0 else "贴合")
        price_line = f"最新价 {price_s}   日内VWAP {state.vwap:.1f} (价{_dev:+.2f}% · {_side})"
    else:
        price_line = f"最新价 {price_s}   日内VWAP N/A"
    print(_box(price_line))
    # VWAP ±σ 通道（迭代五十四 #8）：σ 可用时显示半宽 + 价位（上轨/下轨/带内）
    if state.vwap and state.vwap_sigma is not None and state.vwap_band_pos:
        _half_pct = VWAP_BAND_K * state.vwap_sigma * 100
        print(_box(f"VWAP带 ±{_half_pct:.2f}% ({VWAP_BAND_K}σ) · 价在{state.vwap_band_pos}"))
    # 量价确认（迭代五十一）：本轮增量量 + 均量倍数（放量/缩量），仅展示不计分
    if state.interval_volume is not None:
        if state.vol_surge_ratio is not None:
            _vr = state.vol_surge_ratio
            _vtag = ("放量" if _vr >= VOL_SURGE_MULT
                     else ("缩量" if _vr <= VOL_DIVERGENCE_SHRINK else "均量"))
            vol_line = f"本轮量 {state.interval_volume:,.0f} 股   ({_vr:.1f}× 近均量 · {_vtag})"
        else:
            vol_line = f"本轮量 {state.interval_volume:,.0f} 股   (均量积累中)"
        print(_box(vol_line))
    # 尾盘机构动向（迭代五十二）：仅尾盘窗口（15:30+）显示
    if state.tail_active and state.tail_bias:
        print(_box(f"尾盘 {state.tail_bias}"))
    # 挂单墙迁移 / 冰山补单衰减（迭代五十三）：命中时才显示
    if state.wall_migrate_note:
        print(_box(f"墙迁移 {state.wall_migrate_note}"))
    if state.ice_exhaust_note:
        print(_box(f"冰山 {state.ice_exhaust_note}"))
    # 算法单节奏 / TWAP 匀速（迭代五十四 #3）：匀速命中时显示
    if state.algo_rhythm_note:
        print(_box(f"节奏 {state.algo_rhythm_note}"))
    print(f"╠{bar58}╣")
        ...
        while True:
            ...
            if ret_q == RET_OK and not qdata.empty:
                ...
                if math.isfinite(_lp) and _lp > 0:
                    ...
                    # 当日累计量/额（VWAP + 量价确认共用 get_stock_quote，无额外 API）
                    try:
                        _vol = float(qdata.iloc[0].get("volume", 0) or 0)
                        _to  = float(qdata.iloc[0].get("turnover", 0) or 0)
                    except (KeyError, ValueError, TypeError):
                        _vol = _to = 0.0
                    # 日内 VWAP = 累计成交额 / 累计成交量。机构执行基准 / 日内多空分水岭
                    state.vwap = (_to / _vol) if (_vol > 0 and _to > 0
                                                  and math.isfinite(_to / _vol)) else None
                    # ── 本轮增量成交量（累计差分）+ 均量倍数（迭代五十一 量价确认）──
                    # 累计量每日重置：当前 < 上轮 视为跨日/重置，本轮增量作废并清窗口。
                    _iv: Optional[float] = None
                    if _vol > 0:
                        if state._prev_cum_volume is not None and _vol >= state._prev_cum_volume:
                            _iv = _vol - state._prev_cum_volume
                        elif state._prev_cum_volume is not None and _vol < state._prev_cum_volume:
                            state._vol_window.clear()          # 跨日重置
                        state._prev_cum_volume = _vol
                    state.cum_volume      = _vol if _vol > 0 else None
                    state.interval_volume = _iv
                    # 均量倍数：本轮量 ÷ 近窗口中位量（攒够样本且中位>0 才出；先算后入窗，不含本轮）
                    if len(state._vol_window) >= VOL_BASELINE_MIN_SAMPLES:
                        _base = statistics.median(state._vol_window)
                        state.vol_surge_ratio = (_iv / _base) if (_iv is not None and _base > 0) else None
                    else:
                        state.vol_surge_ratio = None
                    if _iv is not None:
                        state._vol_window.append(_iv)
                        if len(state._vol_window) > VOL_BASELINE_WINDOW:
                            state._vol_window.pop(0)
                    db_save_price(conn, now.isoformat(timespec="seconds"), _lp, _iv)
                    # ── VWAP 穿越事件记录（纯日志，不接入任何评分；供 backtest_signals 验证）──
                    # 带 ±VWAP_CROSS_BAND 死区滤微噪：仅在明确上方↔下方切换时记一次。
                    if state.vwap and state.vwap > 0:
                        _dev = (_lp - state.vwap) / state.vwap
                        _side = 1 if _dev > VWAP_CROSS_BAND else (-1 if _dev < -VWAP_CROSS_BAND else None)
                        if _side is not None:
                            if state._vwap_side is not None and _side != state._vwap_side:
                                if _side > 0:
                                    db_save_signal(conn, "VWAP_CROSS_UP",
                                        f"价上穿日内VWAP {state.vwap:.2f}（{_lp:.2f}, {_dev*100:+.2f}%）", 0)
                                else:
                                    db_save_signal(conn, "VWAP_CROSS_DOWN",
                                        f"价下穿日内VWAP {state.vwap:.2f}（{_lp:.2f}, {_dev*100:+.2f}%）", 0)
                            state._vwap_side = _side
                        # ── VWAP ±σ 通道触轨（迭代五十四 #8，纯记录 score=0）──
                        # dev 滚动 σ → 通道半宽 k·σ；价 ≥+kσ=上轨(回归向下/偏空)、≤−kσ=下轨(回归向上/偏多)。
                        state._vwap_dev_window.append(_dev)
                        if len(state._vwap_dev_window) > VWAP_BAND_WINDOW:
                            state._vwap_dev_window.pop(0)
                        if len(state._vwap_dev_window) >= VWAP_BAND_MIN_SAMPLES:
                            _sigma = statistics.pstdev(state._vwap_dev_window)
                            state.vwap_sigma = _sigma
                            _half = VWAP_BAND_K * _sigma
                            if _sigma > 0 and _dev >= _half:
                                state.vwap_band_pos = "上轨"
                                db_save_signal(conn, "VWAP_BAND_UPPER",
                                    f"价触VWAP上轨（dev{_dev*100:+.2f}% ≥ +{_half*100:.2f}%={VWAP_BAND_K}σ）→偏离过度", 0)
                            elif _sigma > 0 and _dev <= -_half:
                                state.vwap_band_pos = "下轨"
                                db_save_signal(conn, "VWAP_BAND_LOWER",
                                    f"价触VWAP下轨（dev{_dev*100:+.2f}% ≤ -{_half*100:.2f}%={VWAP_BAND_K}σ）→偏离过度", 0)
                            else:
                                state.vwap_band_pos = "带内"
                    else:
                        state.vwap_sigma = None
                        state.vwap_band_pos = None
                    # ── 量价背离事件记录（纯日志 score=0，供 backtest 验证；db_save_price 已写入本轮）──
                    # 价创日内新高/新低却缩量 = 动能/抛压枯竭，经典反转前兆。session_high/low 已含本轮价。
                    if (state.vol_surge_ratio is not None
                            and state.vol_surge_ratio < VOL_DIVERGENCE_SHRINK):
                        _today = datetime.date.today().isoformat()
                        _hi = db_get_session_high(conn, _today)
                        _lo = db_get_session_low(conn, _today)
                        if _hi is not None and _lp >= _hi:
                            db_save_signal(conn, "VOLPRICE_BEAR_DIV",
                                f"价创日内新高{_lp:.2f}但缩量（本轮{_iv:,.0f}股, {state.vol_surge_ratio:.2f}×均量）→动能枯竭", 0)
                        elif _lo is not None and _lp <= _lo:
                            db_save_signal(conn, "VOLPRICE_BULL_DIV",
                                f"价创日内新低{_lp:.2f}但缩量（本轮{_iv:,.0f}股, {state.vol_surge_ratio:.2f}×均量）→抛压枯竭", 0)
                else:
            ...
            # 挂单墙迁移（迭代五十三 #5）：主导墙逐轮上/下移 → 事件记录 + 仪表盘提示
            analyze_wall_migration(conn, state, ob)
            ...
            # 价格证伪闸（迭代五十六，2026-06-18 00100 实盘）：派发末期前提是价弱于机构
            # 执行基准 VWAP；一旦价从下方收复 VWAP 并站稳上方（dev>VWAP_CROSS_BAND），说明
            # 派发供给已被买盘吸收、空头假设证伪。但三条件全锚定日内极值（big_low/small_peak）
            # + 冻结累计值，价格回升永不自撤 → 大单冻结 -2,087 万、派发恒确认，价却 469 收复
            # VWAP 反弹至上轨 +1.3%，安全门通道 B（channel_b=distribution_confirmed）仍逐轮放行
            # 做空=83~100 诱空（买侧冰山吸筹 370×、逼空升至 42 全被顶开）。此处一次性作废：
            # 停掉 +25 主信号分、关闭通道 B 放行、关闭翻转 failsafe 豁免，并清空 sticky 防复活。
            if ((dist_confirmed or state._distribution_sticky_left > 0)
                    and state.vwap and state.last_price and state.vwap > 0
                    and (state.last_price - state.vwap) / state.vwap > VWAP_CROSS_BAND):
                _dev = (state.last_price - state.vwap) / state.vwap * 100
                inval_msg = (f"派发三条件命中但价已收复 VWAP {state.vwap:.1f}"
                             f"（{_dev:+.2f}%，上方偏强）→ 供给被吸收、空头假设证伪，派发确认作废")
                log.warning(f"[派发证伪] {inval_msg}")
                db_save_signal(conn, "DISTRIBUTION_INVALIDATED", inval_msg, 0)
                s_dist, dist_confirmed, sg_dist = 0, False, []
                state._distribution_sticky_left = 0
            ...
            s_ice_sq, s_ice_sup, sg_ice = analyze_iceberg_absorption(conn, state)
            # 强势多头吸筹态闸门：此态下卖侧冰山「派发」多为主力控盘卖墙而非真出货，
            # 反复横跳沦为做空反指（2026-06-18 00100 实盘）→ 屏蔽派发支撑分。须在
            # analyze_broker_footprint 之前清零，连带掐掉它对派发的席位加成(s_brk_sup)。
            if s_ice_sup > 0 and bullish_accum_active(conn, state.last_price, big_net_stale):
                log.info(
                    f"[冰山派发屏蔽] 强势多头吸筹态，卖侧冰山派发 +{s_ice_sup} 分被屏蔽"
                    f"（控盘卖墙非出货，避免反指注水做空）"
                )
                s_ice_sup, sg_ice = 0, []
            # 算法单节奏 / TWAP 匀速（迭代五十四 #3）：事件记录 + 仪表盘提示，不计分
            analyze_algo_rhythm(conn, state)
            ...
            # ── 尾盘机构动向（迭代五十二）：尾盘窗口才计算，更新 state 供仪表盘 + 事件记录 ──
            analyze_closing_behavior(conn, state, datetime.datetime.now())
...
# ═══════════════════════════════════════════════════════════
# probe-scale：实测目标股量级，建议 flow_scale/depth_scale（迭代四十九）
# ═══════════════════════════════════════════════════════════
def cmd_probe_scale(minutes: float = 5.0):
    """盘中采样目标股 N 分钟的成交额 + 卖盘深度，对照 00100 基准算出建议缩放因子。

    - flow_scale  = 目标每分钟成交额 ÷ CALIBRATION_BASELINE['turnover_per_min']
    - depth_scale = 目标卖盘十档深度中位 ÷ CALIBRATION_BASELINE['ask_depth_shares']
    成交额取 get_market_snapshot 的累计 turnover 首尾差；深度每轮采样取中位数(抗噪)。
    须在交易时段运行(否则 turnover 不增长，会提示)。结果直接打印可粘贴的 shared_config 配置块。
    """
    import statistics
    log.info(f"[probe-scale] 采样 {STOCK_NAME}({SYMBOL}) {minutes:g} 分钟，估算建议缩放因子…")
    ctx = OpenQuoteContext(host=OPEND_HOST, port=OPEND_PORT)
    ctx.subscribe([SYMBOL], [SubType.QUOTE, SubType.ORDER_BOOK])

    def _turnover() -> Optional[float]:
        ret, data = ctx.get_market_snapshot([SYMBOL])
        if ret != RET_OK or getattr(data, "empty", True) or "turnover" not in data.columns:
            return None
        return float(data["turnover"].iloc[0])

    def _ask_depth() -> Optional[float]:
        ret, data = ctx.get_order_book(SYMBOL, num=10)
        if ret != RET_OK:
            return None
        return sum(float(it[1]) for it in data.get("Ask", []))

    depths: list[float] = []
    t_start = _turnover()
    clock_start = time.time()
    deadline = clock_start + minutes * 60
    try:
        ...
        log.info("[probe-scale] 手动中断，用已采样数据估算")
    t_end = _turnover()
    elapsed_min = (time.time() - clock_start) / 60
    ctx.close()

    if not depths or t_start is None or t_end is None:
        log.error("[probe-scale] 采样失败（未取到深度或成交额）——确认 OpenD 已启动且在交易时段")
        return

    turnover_delta = t_end - t_start
    if turnover_delta <= 0 or elapsed_min <= 0:
        log.warning(f"[probe-scale] 成交额无增长（Δturnover={turnover_delta:,.0f}）——"
                    f"很可能非交易时段或停牌，flow_scale 不可信，仅供参考")
    turnover_per_min = turnover_delta / elapsed_min if elapsed_min > 0 else 0.0
    depth_median = statistics.median(depths)

    base_to = CALIBRATION_BASELINE["turnover_per_min"]
    base_dep = CALIBRATION_BASELINE["ask_depth_shares"]
    flow_scale  = turnover_per_min / base_to if base_to else 0.0
    depth_scale = depth_median / base_dep if base_dep else 0.0
    _r = lambda x: round(x, 1) if x >= 1 else round(x, 2)

    print("\n" + "═" * 60)
    print(f" probe-scale 结果：{STOCK_NAME}（{SYMBOL}）  采样 {elapsed_min:.1f} 分钟 / {len(depths)} 个深度样本")
    print("═" * 60)
    print(f" 实测每分钟成交额 : {turnover_per_min:>14,.0f} HKD   (00100 基准 {base_to:,.0f})")
    print(f" 实测卖盘深度中位 : {depth_median:>14,.0f} 股     (00100 基准 {base_dep:,.0f})")
    print("─" * 60)
    print(f" 建议 flow_scale  : {_r(flow_scale)}")
    print(f" 建议 depth_scale : {_r(depth_scale)}")
    print("─" * 60)
    print(" 粘贴进 shared_config.py 的 STOCKS[该股] ：")
    print(f'        "flow_scale":  {_r(flow_scale)},')
    print(f'        "depth_scale": {_r(depth_scale)},')
    print("═" * 60)
    print(" ⚠ 仅量级对齐，不保证信号迁移有效——启用前务必 backtest_signals.py 验证。")
    print(" ⚠ 单次采样含随机性，建议不同时段多采几次取稳。\n")


# ═══════════════════════════════════════════════════════════
# Per-stock 阈值校准（迭代四十九）
# ═══════════════════════════════════════════════════════════
def apply_stock_calibration(code: str) -> list[str]:
    """按 STOCKS[code] 的 flow_scale/depth_scale + calibration 覆盖本模块的 per-stock 量级阈值。

    本模块顶部的阈值常量是 00100 的「默认 profile」。本函数在启动选定股票后，用缩放因子
    就地改写这些全局(globals() 在模块级函数里返回模块命名空间，故直接 mutate 模块常量)；
    所有阈值都在分析函数体内按调用时读取，启动期改写一次即对全程生效。
    `CALIBRATABLE` 白名单防止 config 里的错误 KEY 静默篡改无关全局。
    返回被改动项的说明列表(供启动日志)。00100 或未配缩放/覆盖的股票返回空列表(行为与改造前完全一致)。
    """
    cfg = STOCKS.get(code, {})
    flow_scale  = float(cfg.get("flow_scale", 1.0))
    depth_scale = float(cfg.get("depth_scale", 1.0))
    overrides   = dict(cfg.get("calibration", {}) or {})
    g = globals()
    changed: list[str] = []

    # ① 缩放(显式覆盖的项跳过，留给 ② 处理，避免被缩放又被覆盖两次)
    for key, kind in CALIBRATABLE.items():
        if key in overrides or key not in g:
            continue
        scale = flow_scale if kind == "flow" else depth_scale
        if scale == 1.0:
            continue
        old = g[key]
        g[key] = int(round(old * scale))
        changed.append(f"{key}: {old:,} → {g[key]:,} (×{scale:g} {kind})")

    # ② 显式逐项覆盖(在缩放之后生效)
    for key, val in overrides.items():
        if key not in CALIBRATABLE:
            log.warning(f"[校准] 忽略未知校准键 {key!r}（不在 CALIBRATABLE 白名单）")
            continue
        if key not in g:
            log.warning(f"[校准] 校准键 {key!r} 在本模块无对应常量，跳过")
            continue
        old = g[key]
        g[key] = val
        changed.append(f"{key}: {old:,} → {val:,} (显式覆盖)")
...
"""
    ...
    _p.add_argument("cmd", nargs="?", default="monitor",
                    choices=["monitor", "backfill", "signals", "export", "probe-scale"],
                    help="子命令（默认 monitor）")
    _p.add_argument("--minutes", type=float, default=5.0,
                    metavar="MIN", help="probe-scale 采样时长（分钟，默认 5）")
    ...
    # 应用 per-stock 量级阈值校准（缩放/覆盖 00100 默认 profile，迭代四十九）
    _cal_changes = apply_stock_calibration(_args.stock)
    if _cal_changes:
        log.info(f"[校准:{_args.stock}] 覆盖 {len(_cal_changes)} 项量级阈值（基准 00100）：")
        for _c in _cal_changes:
            log.info(f"  ↳ {_c}")
    elif _args.stock != DEFAULT_STOCK:
        log.warning(f"[校准:{_args.stock}] 未配 flow_scale/depth_scale/calibration —— "
                    f"沿用 00100 量级阈值，非 00100 同类标的数据可能失真，建议先标定")
    ...
    if _args.cmd == "monitor":
        ...
        cmd_backfill()
    elif _args.cmd == "probe-scale":
        cmd_probe_scale(minutes=_args.minutes)
