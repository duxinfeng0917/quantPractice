...
def analyze_short_entry(
    ...
    安全门设计（三元 AND）：
        BLOCKED 触发条件 = 逼空分高 AND (价格非确认下行) AND (大单非持续净流出)
        即：当逼空分高但日内价格已确认下行且资金面持续净流出时，认为逼空假设
        已被价格行为否定，允许开空。这是为避免 HKEX 日级静态信号（成本线、
        高位拐头）锁死整个交易日的入场窗口。
    """
    ...
    if effective_squeeze > SHORT_SAFE_SQUEEZE:
        # 检查放行条件：价格已确认下行 + 大单持续净流出 → 行情否定逼空，放行
        ...
        if price_confirmed_down and bigflow_confirmed_out:
            # 放行：行情已确认下行 + 资金持续净流出，逼空假设被价格行为否定
            override_msg = (
